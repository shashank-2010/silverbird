...
class ModelEvaluator:
    ...
    def evaluate(self, model, X_test, y_test):
        ...
        #direction_acc = np.mean((np.sign(np.diff(y_test)) == np.sign(np.diff(preds)))[:-1])
        def classify_direction(change, threshold=0.001):
            return np.where(change > threshold, 1,
                np.where(change < -threshold, -1, 0))

        actual_dir = classify_direction(np.diff(y_test))
        pred_dir = classify_direction(np.diff(preds))
        ...
        return {
            "MSE": round(mse, 4),
            "MAE": round(mae, 4),
            "Directional Accuracy": round(direction_acc * 100, 2)
        }
